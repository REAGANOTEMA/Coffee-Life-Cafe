...
// ==================== CONTINUOUS TITLE SLIDE ====================
if (title && logoContainer) {
    let posX = 0;
    const slideSpeed = 0.6;
    function slideTitle() {
        const headerWidth = header.offsetWidth;
        const logoWidth = logoContainer.offsetWidth + 20;
        const navWidth = Array.from(navLinks).reduce((sum, link) => sum + link.offsetWidth + 20, 0);
        const maxSlide = headerWidth - logoWidth - navWidth - title.offsetWidth;
        posX += slideSpeed;
        if (posX > maxSlide) posX = 0;
        title.style.transform = `translateX(${posX}px)`;
        requestAnimationFrame(slideTitle);
    }
    slideTitle();
}
```
